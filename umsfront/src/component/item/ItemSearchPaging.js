...
function ItemSearchPaging(props) {
    ...
    function onClickHandler(pageNum) {
        fetchFn(
            "GET",
            `http://localhost:9007/api/item/list/username/search?username=${username}&pageNum=${pageNum - 1}`
        ).then((data) => {
    ...
    function getPageNumInfo() {
        fetchFn(
            "GET",
            `http://localhost:9007/api/item/list/username/search?username=${username}&pageNum=0`
        ).then((data) => {
            setTotalPages(data.result.totalPages);
        });
    }
