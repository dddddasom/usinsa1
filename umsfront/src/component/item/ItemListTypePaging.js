...
function ItemListPaging(props) {
    ...
    function onClickHandler(pageNum) {
        fetchFn(
            "GET",
            `http://localhost:9007/api/item/itemtype/itemtype?itemType=${itemType}&pageNum=${pageNum - 1}`
        ).then((data) => {
    ...
    function getPageNumInfo() {
        fetchFn(
            "GET",
            `http://localhost:9007/api/item/itemtype/itemtype?itemType=${itemType}&pageNum=0`
        ).then((data) => {
            setTotalPages(data.result.totalPages);
